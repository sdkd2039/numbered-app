...
class DhikrWidget : AppWidgetProvider() {  
    ...
    private fun increment(  
        ...
        // Defensive: the stored target may be 0/negative if data ever  
        // changes; never let it break coerceIn/progress-bar math below.  
        val target =  
            item.count.coerceAtLeast(1)  
  
        // Defensive: sanitize a possibly stale/negative stored count  
    ...
    private fun move(  
        ...
        // Defensive: sanitize a possibly stale/out-of-range stored index  
        // before applying direction (consistent with increment()/updateWidget()).  
        var index =  
            prefs.getInt(  
                KEY_INDEX_PREFIX + widgetId,  
                0  
    ...
    private fun updateWidget(  
        ...
        // Defensive: never let a 0/negative target reach coerceIn or the  
        // progress bar's max.  
        val target =  
            item.count.coerceAtLeast(1)  
